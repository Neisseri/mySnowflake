...
int main(int argc,char* argv[]){
    ...
    CHECK_ERROR(cudaMallocManaged((void**)&ratio, sizeof(float) * dimX * dimY));
    ...
    for(int i=0;i<timesteps;i++){
        ...
        kernel1_pure<<<grids,blocks>>>(phi,phi_lap,tempr,tempr_lap,phidx,phidy,epsilon,epsilon_deri, ratio);
        kernel2_pure<<<grids,blocks>>>(phi,phi_lap,epsilon,epsilon_deri,phidx,phidy,tempr,tempr_lap, ratio);
        cudaDeviceSynchronize();
        if (i % 50 == 0) std::cout << *ratio << " ";
    }
    // 每 50 轮测量一次比值
